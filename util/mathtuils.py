    ...
    for idx, point in enumerate(profile):
        ...
        if x1 <= x and x <= x2:
            if idx+1 < len(profile):
                return interpolate(point, profile[idx+1],
                                   (x-x1)/(x2-x1), method)
            else:
                return interpolate(point, profile[idx],
                                   (x-x1)/(x2-x1), method)
    return 1
        ...
        for iy in range(0, len(profile[ix])):
            ...
            if x1 <= x and x <= x2 and y1 <= y and y <= y2:
                if ix+1 < len(profile) or iy+1 < len(profile[ix]):
                    i1 = interpolate(profile[ix][iy], profile[ix+1][iy],
                                     (x-x1)/(x2-x1), method)
                    i2 = interpolate(profile[ix][iy+1], profile[ix+1][iy+1],
                                     (x-x1)/(x2-x1), method)
