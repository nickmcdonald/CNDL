...
from qtpy.QtWidgets import (QWidget, QFrame, QApplication, QLabel, QPushButton,
...
class MathOperationDataModel(NodeDataModel):
	...
	def _check_inputs(self):
		ies1_ok = (self._ies1 is not None and
					  self._ies1.data_type.id in ('ies'))
		ies2_ok = (self._ies2 is not None and
					  self._ies2.data_type.id in ('ies'))
...
class IesDisplayModel(NodeDataModel):
	...
	def set_in_data(self, data: NodeData, port: Port):
		...
		'''
		self._ies = data
		ies_ok = (self._ies is not None and
					 self._ies.data_type.id in ('ies'))
